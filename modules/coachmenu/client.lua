...
RegisterNetEvent("rpx-doctorjob:client:CoachMenu",function()
    lib.showContext('doctor_coach_menu')
end)
...
lib.registerContext({
    id = 'doctor_coach_menu',
    title = 'Doctor Coaches',
    options = {
        {
            title = 'Spawn Doctor Buggy',
            description = 'Spawn a doctor buggy. If you have one out already, it will be despawned.',
            icon = 'horse',
            onSelect = function()
                ...
                local model = GetHashKey("buggy02")
        ...
        {
            title = 'Despawn Buggy',
            description = 'Despawn your current buggy.',
            icon = 'trash-can',
            onSelect = function()
                if playerCoachId then
                    if DoesEntityExist(playerCoachId) then
...
Citizen.CreateThread(function()
    while true do
        ...
        hasPermission = exports['rpx-core']:HasJobPermission(plyState.job?.name, plyState.job?.rank, "doctor:general")
        ...
        if hasPermission then
            if not PromptSet then
                for id,spawner in pairs(Config.CoachSpawners) do
                    exports['rpx-core']:createPrompt('doctor_coach_spawner_'..id, spawner.MenuPosition, 0xD9D0E1C0, 'Coach Spawner', {
                        func = function()
                            if GetGameTimer() - PromptTimeout > 1000 then
                                PromptTimeout = GetGameTimer()
                                TriggerServerEvent("rpx-doctorjob:server:RequestCoachMenu")
                            end
                        end
                    })
                end
            ...
            if PromptSet then
                for id,spawner in pairs(Config.CoachSpawners) do
                    exports['rpx-core']:deletePrompt('doctor_coach_spawner_'..id)
                end
                PromptSet = false
            end
        end
